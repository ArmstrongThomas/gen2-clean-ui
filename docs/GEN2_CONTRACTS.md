...
Current implementation status: all 41 supported records are integrated into
product composition, production Gallery conversion, and aggregate tests. The
Gen2 battle presenter covers the source state-machine envelope, including
command/move/message, forced-switch/next-Pokémon, move learning, stats,
progression, evolution, nickname, item, Poké Ball, and detached animation
frames. Supported Party, Pack, and naming children compose over that envelope;
unknown or unsupported nested stacks remain native whenever the complete stack
cannot be proven. Hall of Fame is viewer/display-only and its induction phase
remains native. Shared `gold.CallerBox` remains native/pending because the
public host API has no proven exact identity seam.
...
| 2 | `Gen2BattleState` | Supported: full phase envelope | 1.0.0 | BATTLE |
| 3 | `Gen2BattleTransition` | Supported: transparent V3 overlay | 1.0.0 | ANIMATION |
...
Save, Battle, and the extracted cinematic presenters—now emit the canonical
...
The 41 integrated official production presenters now require the canonical V3
model at the provider boundary, and the shared renderer repeats that check at
its final pre-measure/render boundary; the remaining official records are
native by design. Egg Hatch uses source-owned beat timing with extracted egg,
hatchling, crack, and fragment art. Evolution uses the same seam for
source-authored flash/reveal sprites and expanding light circles. Gold/Silver
Intro, the Copyright splash, Game Freak Presents, and the title screen remain
native until their raster seams are proven. The battle transition uses
`animation.overlay = true` so
its clean effect composes over the source-owned world without inventing a
centered panel or claiming ownership of the world renderer.
