...
local function screenId(state)
  return type(state) == "table" and rawget(state, "screenId") or nil
end

local function containsScreen(states, wanted)
  if type(states) ~= "table" then return false end
  for _, state in ipairs(states) do
    if screenId(state) == wanted then return true end
  end
  return false
end

local function battleToken(states)
  if type(states) ~= "table" then return nil end
  for _, state in ipairs(states) do
    if screenId(state) == "Gen2BattleState" then
      local battle = rawget(state, "battle")
      -- BattleState instances are rebuilt while an intro, move, or finishing
      -- hit is resolving.  Do not compare the table identity: that would
      -- release the presentation latch exactly when the native renderer is
      -- most likely to flash.  Prefer an explicit encounter token when the
      -- source exposes one and otherwise use the stable screen identity.
      local id = rawget(state, "battleId") or rawget(state, "encounterId")
        or (type(battle) == "table" and (rawget(battle, "battleId")
          or rawget(battle, "encounterId") or rawget(battle, "id")))
      return id == nil and screenId(state)
        or (screenId(state) .. ":" .. tostring(id))
    end
  end
  return nil
end

local function hiddenScreenIds(entries)
  local ids = {}
  for _, entry in ipairs(entries or {}) do
    local id = screenId(entry.state)
    if id and entry.prepared and entry.prepared.suppress == true
        then
      ids[id] = true
    end
  end
...
function Runtime.new(core)
  local self = { core=core, mod=core.mod, provider=core.provider,
    ...
    frameSerial=0, lastBattleCandidate=nil, lastGame=nil }
  ...
  function self:prepare(game, viewport)
    local previousBattle = self.lastBattleCandidate
    self:clear("prepare")
    self.frameId = self.frameId + 1
    local graphics = love and love.graphics
    local battleVisible, currentBattleToken = false, nil
    local function failed(reason)
      -- Official battle animations briefly expose incomplete battler/OAM
      -- snapshots while the source state is still the owner of the battle.
      -- Keep the last complete Clean frame in that narrow window. This is a
      -- presentation latch only: source update, input, timing, and audio
      -- continue to run normally, and the latch is discarded as soon as the
      -- visible stack is no longer the same battle.
      -- A host refresh may rebuild the presentation at a different window
      -- size before the next complete V3 snapshot is available.  Viewport
      -- changes are not a battle boundary: releasing ownership here exposes
      -- the native battle canvas precisely when the host is redrawing it.
      -- The next complete prepare re-solves the detached layout; until then
      -- the last complete Clean frame remains the safe presentation owner.
      local terminalBattle = tostring(reason):match("^battle_finished") ~= nil
      local sameBattle = previousBattle and previousBattle.battleOwned
        and battleVisible
        and not terminalBattle
        and (not previousBattle.battleToken
          or not currentBattleToken
          or previousBattle.battleToken == currentBattleToken)
      if sameBattle then
        -- A host refresh can hand the hook a recreated game facade while the
        -- visible battle identity is unchanged.  Retarget the retained
        -- candidate so render.hud still composes the detached frame for the
        -- refreshed facade.
        previousBattle.game = game
        previousBattle.frameSerial = self.frameSerial
        previousBattle.latched = true
        previousBattle.latchReason = tostring(reason)
        self.candidate = previousBattle
        self.lastReason = "battle_latched:" .. tostring(reason)
        return self.candidate
      end
      -- A failed non-battle preparation, a changed battle token, or an
      -- unknown stack must not leave an old battle frame eligible for a later
      -- encounter.
      self.lastBattleCandidate = nil
      self.lastReason = reason
      return nil, reason
    ...
    battleVisible = containsScreen(states, "Gen2BattleState")
    currentBattleToken = battleToken(states)
    local w, h = vp.w, vp.h
    ...
    local candidateBattleToken
    for _, entry in ipairs(entries) do
      if entry.model and entry.model.kind == "battle" then
        ownsBattle = true
        candidateBattleToken = battleToken({ entry.state })
        break
      end
    end
    self.candidate = { game=game, viewport=vp, entries=entries,
      hidden=hidden, hiddenScreenIds=hiddenScreenIds(entries), canvas=canvas,
      frameSerial=self.frameSerial, battleOwned=ownsBattle,
      battleToken=candidateBattleToken, latched=false }
    if ownsBattle then
      self.lastBattleCandidate = self.candidate
    else
      self.lastBattleCandidate = nil
    end
  ...
  function self:install()
    ...
    if self.mod.events and self.mod.events.on then
      ...
      subscriptions[#subscriptions + 1] = self.mod.events:on(
        -- A battle child can be pushed while the battle state remains the
        -- source owner.  Drop only the current rendered candidate here; keep
        -- the last complete battle frame available for the next prepare to
        -- latch across a rebuilt/transient BattleState snapshot.  The
        -- prepare path still releases it when the battle token disappears.
        "screen.pushed", function() self:clear("screen_pushed") end,
        90000)
      subscriptions[#subscriptions + 1] = self.mod.events:on(
        "screen.popped", function() self:clear("screen_popped") end,
        90000)
    end
    ...
    subscriptions[#subscriptions + 1] = self.mod.hooks:wrap("screen.render_visible",
      function(nextFn, state)
        ...
        if candidate and (candidate.hidden[state]
            or (candidate.hiddenScreenIds
              and candidate.hiddenScreenIds[screenId(state)])) then
          return false
        end
        return visible
      end, 90000)
