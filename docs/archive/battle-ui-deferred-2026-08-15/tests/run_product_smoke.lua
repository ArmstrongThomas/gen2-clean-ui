...
local classes = {}
for _, id in ipairs({ "Gen2MainMenu", "Gen2OptionsMenu", "Gen2BattleState",
...
package.preload[contracts.Gen2BattleState.module] = function()
  return classes.Gen2BattleState
end
...
if not headless then
  ...
  local liveBattle = setmetatable({
    game = fakeGame, screenId = "Gen2BattleState", phase = "menu",
    slideFrame = 72, showPlayerHud = true, showEnemyHud = true,
    picHidden = { player=true, enemy=true },
    pokemon = {
      CYNDAQUIL = { spriteBack = "assets/generated/battle/back/cyndaquil.png" },
      PIDGEY = { spriteFront = "assets/generated/battle/front/pidgey.png" },
    },
    battle = {
      wild = true,
      player = { species = "CYNDAQUIL", nickname = "CYNDAQUIL",
        level = 12, hp = 34, maxHp = 34, moves = {} },
      enemy = { species = "PIDGEY", name = "PIDGEY", level = 4,
        hp = 18, maxHp = 18 },
    },
  }, classes.Gen2BattleState)
  fakeStack.states = { liveBattle }
  hookWrappers["render.ui.prepare"](function() end, fakeGame,
    { width=1280, height=720 })
  assert(hookWrappers["screen.render_visible"](
    function() return true end, liveBattle) == false,
    "detached battle suppresses the native source scene")
  assert(hookWrappers["render.compose"] == nil,
    "detached battle does not install a native source-canvas renderer")

  -- An active runner can expose a transient V3 gap between extracted frames.
  -- The previous detached clean candidate must latch without releasing native
  -- ownership or inventing a text-only animation substitute.
  liveBattle.phase = "resolving"
  liveBattle.anim = {
    env = { animId = "ANIM_TACKLE", battleTurn = 1 },
  ...
  assert(hookWrappers["screen.render_visible"](
    function() return true end, liveBattle) == false,
    "transient animation keeps detached ownership latched")

  -- The released state can also rebuild before the active battler snapshot is
  -- repopulated. Detached ownership survives that gap with the last complete
  -- clean frame; the native canvas is never used as a fallback image.
  liveBattle.battle.player = nil
  liveBattle.anim = nil
  hookWrappers["render.ui.prepare"](function() end, fakeGame,
    { width=1280, height=720 })
  assert(hookWrappers["screen.render_visible"](
    function() return true end, liveBattle) == false,
    "detached ownership survives an incomplete battler snapshot")
  -- Resizing the official window is also a host redraw/refresh boundary.  It
  -- must not release a valid battle owner merely because the new viewport is
  -- prepared while the source is between V3 snapshots.
  hookWrappers["render.ui.prepare"](function() end, fakeGame,
    { width=1024, height=768 })
  assert(hookWrappers["screen.render_visible"](
    function() return true end, liveBattle) == false,
    "battle ownership survives a refresh at a new viewport")
  local refreshedGame = { stack = { states = { liveBattle } } }
  liveBattle.game = refreshedGame
  hookWrappers["render.ui.prepare"](function() end, refreshedGame,
    { width=1024, height=768 })
  assert(hookWrappers["screen.render_visible"](
    function() return true end, liveBattle) == false,
    "battle ownership survives a refreshed game facade")
  liveBattle.game = fakeGame
  liveBattle.anim = nil
  liveBattle.phase = "menu"

  -- `done` is an explicit source-owned battle boundary even if the host keeps
  -- the state on the stack for one cleanup frame; do not latch an old battle
  -- canvas into the next overworld/progression frame.
  liveBattle.phase = "done"
  hookWrappers["render.ui.prepare"](function() end, fakeGame,
    { width=1024, height=768 })
  assert(hookWrappers["screen.render_visible"](
    function() return true end, liveBattle) == true,
    "terminal battle phase releases detached ownership")
  liveBattle.phase = "menu"
